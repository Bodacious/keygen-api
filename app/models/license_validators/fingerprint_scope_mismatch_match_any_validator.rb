...
module LicenseValidators
  class FingerprintScopeMismatchMatchAnyValidator
    ...
    def invalid?
      @fingerprints.size > 1 && @policy.machine_match_any? && @machines.alive.count.zero?
    end

    def failure_result
      [false,
       "one or more fingerprint is not activated (does not match any associated machines)", :FINGERPRINT_SCOPE_MISMATCH]
    end
  end
end
