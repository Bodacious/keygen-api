...
class LicenseValidationService
  ...
  def validate!
    ...
    if scope.present? && scope.key?(:components)
      ...
      return [false, "components scope is empty", :COMPONENTS_SCOPE_EMPTY] if fingerprints.empty?
      ...
      when license.policy.component_match_most?
        return [false, "one or more component is not activated (does not match enough associated components)", :COMPONENTS_SCOPE_MISMATCH] if components.count < (fingerprints.size / 2.0).ceil
      when license.policy.component_match_two?
        return [false, "one or more component is not activated (does not match at least 2 associated components)", :COMPONENTS_SCOPE_MISMATCH] if components.count < 2
      when license.policy.component_match_all?
        return [false, "one or more component is not activated (does not match all associated components)", :COMPONENTS_SCOPE_MISMATCH] if components.count < fingerprints.size
      else
        return [false, "one or more component is not activated (does not match any associated components)", :COMPONENTS_SCOPE_MISMATCH] if components.count == 0
      end
    else
    ...
    unless license.policy.strict?
      ...
      return [license.allow_access? || license.maintain_access?, "is expired", :EXPIRED] if license.expired?

      return [true, "is valid", :VALID]
    end

    # Check if license policy allows floating and if not, should have single activation
    return [false, "must have exactly 1 associated machine", :NO_MACHINE] if license.node_locked? && license.machines_count == 0
    ...
    if license.node_locked? && license.machines_count > 1
      ...
      return [allow_overage, "has too many associated machines", :TOO_MANY_MACHINES] if machine_count > machine_limit
    end

    # When floating, license should have at least 1 activation
    return [false, "must have at least 1 associated machine", :NO_MACHINES] if license.floating? && license.machines_count == 0
    ...
    if license.max_cores?
      ...
      return [allow_overage, "has too many associated machine cores", :TOO_MANY_CORES] if core_count > core_limit
    end
    ...
    if license.max_processes?
      ...
      return [allow_overage, "has too many associated processes", :TOO_MANY_PROCESSES] if process_count > process_limit
    end
  ...
  def touch!
    return if skip_touch? || license.nil? || touches.empty?
